...
Bytes:十六进制类型，编码转换，离计算机二进制最近的数据类型
...
str in str2 查看某字符串是否在另一个字符串中包含 "el" in "Hello"

str + str2 字符串拼接,禁用 每有一个字符串就开辟一块对应内存 效率太低
...
当 b='he's happy \n Yes' 想要正常一行输出则转变为
    ...
    b=r'he\'s happy \n Yes' 前面 + r 可以一行输出 's 可以用\转义 当然也可以利用"" """ """

那么如果以上是用户输入的话的 这里利用 repr 来解决 使其一行输出
...
#####1.万恶的 + 号：(禁用！！)：
字符串用加号的形式来拼接字符串 "str1" + "str2" 但没人这么用，因为字符串是不可变类型

        例: print("My name is "+name+" and i am "+age+" years old.")

在计算机内存中开辟了五块内存存储字符串，效率低
...
但通常推荐 format 代替 % :
    ...
    例：
        foo = "2"
        bar = "33"
        msg = '{a}{b}'.format(a=foo,b=bar) 不过嫌麻烦也经常写为 '{0}{1}'.format(foo, bar)

[注]：配置 DNS bind 配置文件 zone 时遇到的坑
...
当利用 format 遇到字符串中含有花括号时： {} 利用 {{}} 将它代替

[缺点]：在处理多个参数和更长的字符串时仍然可能非常冗长

##### 3.join 拼接：（这是字符串特有的操作）

"".join(list/tuple) 字符串按""的方式拼接 url 之类的常用 重要 应熟练使用
...
##### 4.通过 F-strings 拼接: (在 python3.6.2 版本中，PEP 498 提出)
